...
class Story extends Model
{
    ...
    public static function getListNewStories($categoryID = 0)
    {
        ...
        {
            $category = Category::where('id', $categoryID)->first();
            $stories = $category->stories()->orderBy('updated_at', 'DESC')->skip(0)->take(20)->get();
        }
        else
            $stories = Story::orderBy('updated_at', 'DESC')->skip(0)->take(20)->get();
    ...
    {
        $stories = self::where('status', 1)->orderBy('updated_at', 'DESC')->skip(0)->take(24)->get();
